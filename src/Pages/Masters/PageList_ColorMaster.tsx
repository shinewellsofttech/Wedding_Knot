...
const LIST_API_URL = `${API_WEB_URLS.MASTER}/0/token/ColorMaster/Id/0`;
const DELETE_API_URL = `${API_WEB_URLS.MASTER}/0/token/ColorMaster`;
...
/**
 * Lists color masters with search and CRUD support.
 */
const PageList_ColorMaster = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const [state, setState] = useState<ColorListState>({
    ColorMasterList: [],
    isProgress: true,
    filterText: "",
  });
  ...
  /**
   * Loads color master list.
   */
  const loadData = useCallback(() => {
    setState((prev) => ({ ...prev, isProgress: true }));
    Fn_FillListData(dispatch, setState, "ColorMasterList", LIST_API_URL).catch((error) => {
      console.error("Failed to load color masters:", error);
   ...
   */
  const handleAdd = () => {
    navigate("/addEditColorMaster", { state: { Id: 0 } });
  };
  ...
  const handleEdit = (id: number | string) => {
    if (!id) return;
    navigate("/addEditColorMaster", { state: { Id: id } });
  };

  /**
   * Delete selected color master.
   */
  const handleDelete = (id: number | string) => {
    ...
    if (window.confirm(`Are you sure you want to delete '${itemName}'?`)) {
      Fn_DeleteData(dispatch, setState as any, Number(id), DELETE_API_URL, LIST_API_URL)
        ...
        .catch((error) => {
          console.error("Failed to delete color master:", error);
          alert("Failed to delete color master. Please try again.");
        });
    }
  };
  ...
  const filteredList = useMemo(() => {
    const rawList = Array.isArray(state.ColorMasterList) ? state.ColorMasterList : [];
  ...
  }, [state.ColorMasterList, state.filterText]);
      ...
      <div className="page-body">
        ...
        <Container fluid>
          <Row>
            <Col xs="12">
              <Card>
                <CardHeaderCommon title="Color Master List" tagClass="card-title mb-0" />
                <CardBody>
                  <Row className="mb-3">
...
export default PageList_ColorMaster;
