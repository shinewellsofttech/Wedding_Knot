...
const LIST_API_URL = `${API_WEB_URLS.MASTER}/0/token/ItemTypeMaster/Id/0`;
const DELETE_API_URL = `${API_WEB_URLS.MASTER}/0/token/ItemTypeMaster`;

interface ItemTypeListState {
  ItemTypeMasterList: any[];
  isProgress: boolean;
  filterText: string;
}

/**
 * Lists item types with search and CRUD support.
 */
const PageList_ItemType = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const [state, setState] = useState<ItemTypeListState>({
    ItemTypeMasterList: [],
    isProgress: true,
    filterText: "",
  });
  ...
  /**
   * Loads item type list.
   */
  const loadData = useCallback(() => {
    setState((prev) => ({ ...prev, isProgress: true }));
    Fn_FillListData(dispatch, setState, "ItemTypeMasterList", LIST_API_URL).catch((error) => {
   ...
   */
  const handleAdd = () => {
    navigate("/addEditItemType", { state: { Id: 0 } });
  };
  ...
  const handleEdit = (id: number | string) => {
    if (!id) return;
    navigate("/addEditItemType", { state: { Id: id } });
  };
  ...
  const handleDelete = (id: number | string) => {
    if (!id) return;
    const itemToDelete = state.ItemTypeMasterList?.find((item: any) => String(item?.Id) === String(id));
  ...
  const filteredList = useMemo(() => {
    const rawList = Array.isArray(state.ItemTypeMasterList) ? state.ItemTypeMasterList : [];
  ...
  }, [state.ItemTypeMasterList, state.filterText]);
      ...
      <div className="page-body">
        ...
        <Container fluid>
          <Row>
            <Col xs="12">
              <Card>
                <CardHeaderCommon title="Item Type List" tagClass="card-title mb-0" />
                <CardBody>
                  <Row className="mb-3">
...
export default PageList_ItemType;
