...
/**
 * Add/Edit form for Category master.
 */
const AddEdit_CategoryMaster = () => {
  ...
  const [categoryMasterState, setCategoryMasterState] = useState<CategoryMasterState>({
    id: 0,
    formData: { ...initialValues },
    isProgress: false,
  });

  const isEditMode = categoryMasterState.id > 0;

  /**
   * Validation schema for category master form.
   */
  ...
  useEffect(() => {
    ...
    if (recordId > 0) {
      setCategoryMasterState((prev) => ({
        ...prev,
        id: recordId,
      }));
      Fn_DisplayData(dispatch, setCategoryMasterState, recordId, API_URL_EDIT);
    } else {
      setCategoryMasterState((prev) => ({
        ...prev,
        id: 0,
  ...
  const initialFormValues: FormValues = {
    ...initialValues,
    Name: toStringOrEmpty(categoryMasterState.formData.Name || (categoryMasterState.formData as any).CategoryName),
  };

  /**
   * Submit handler for category master.
   */
  const handleSubmit = async (values: FormValues, { setSubmitting }: FormikHelpers<FormValues>) => {
    try {
      const formData = new FormData();
      formData.append("Id", String(categoryMasterState.id ?? 0));
      ...
      await Fn_AddEditData(
        dispatch,
        () => undefined,
        { arguList: { id: categoryMasterState.id, formData } },
        API_URL_SAVE,
        true,
        "memberid",
        navigate,
      ...
      console.error("Category master submission failed:", error);
    } finally {
      setSubmitting(false);
    }
  };
                  ...
                  <Form className="theme-form" onKeyDown={handleEnterToNextField}>
                    <Card>
                      ...
                      <CardFooter className="text-end">
                        <Btn color="secondary" type="button" className="me-2" onClick={() => navigate("/categoryMaster")}>
                          Cancel
                        </Btn>
