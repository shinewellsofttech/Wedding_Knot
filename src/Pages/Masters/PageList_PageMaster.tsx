...
const DELETE_API_URL = `${API_WEB_URLS.MASTER}/0/token/PageMaster`;
...
/**
 * Lists page masters with search and CRUD support.
 */
const PageList_PageMaster = () => {
  ...
  /**
   * Loads page master list.
   */
  const loadData = useCallback(() => {
    setState((prev) => ({ ...prev, isProgress: true }));
    Fn_FillListData(dispatch, setState, "PageMasterList", LIST_API_URL).catch((error) => {
      console.error("Failed to load page masters:", error);
   ...
   */
  const handleAdd = () => {
    navigate("/addEditPageMaster", { state: { Id: 0 } });
  };
  ...
  const handleEdit = (id: number | string) => {
    if (!id) return;
    navigate("/addEditPageMaster", { state: { Id: id } });
  };

  /**
   * Delete selected page master.
   */
  const handleDelete = (id: number | string) => {
    if (!id) return;
    const itemToDelete = state.PageMasterList?.find((item: any) => String(item?.Id) === String(id));
    ...
    if (window.confirm(`Are you sure you want to delete '${itemName}'?`)) {
      Fn_DeleteData(dispatch, setState as any, Number(id), DELETE_API_URL, LIST_API_URL)
        ...
        .catch((error) => {
          console.error("Failed to delete page master:", error);
          alert("Failed to delete page master. Please try again.");
        });
    }
  };
  ...
  const filteredList = useMemo(() => {
    const rawList = Array.isArray(state.PageMasterList) ? state.PageMasterList : [];
    ...
    return rawList.filter((item) => {
      const fields = [item?.Name, item?.PageType];
      return fields.some((field) => String(field ?? "").toLowerCase().includes(searchText));
    });
  }, [state.PageMasterList, state.filterText]);
      ...
      <div className="page-body">
        ...
        <Container fluid>
          <Row>
            <Col xs="12">
              <Card>
                <CardHeaderCommon title="Page Master List" tagClass="card-title mb-0" />
                <CardBody>
                  <Row className="mb-3">
                    <Col md="6" className="d-flex align-items-center">
                      ...
                      <Input
                        type="search"
                        placeholder="Search by page name or page type..."
...
export default PageList_PageMaster;
