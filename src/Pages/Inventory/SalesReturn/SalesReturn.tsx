...
import GridSystemSalesReturn from "./GridSystemSalesReturn";
...
interface GridRow {
  ...
  ItemData: any[] | null;
  AvailableQty?: number;
  F_SalesInvoiceH?: string | number;
  F_SalesInvoiceL?: string | number;
...
interface StateData {
  ...
  formData: {
    ...
    F_SalesReturnH?: string;
  };
  CreatedSalesInvoices?: any[];
  SalesInvoiceLinesMap?: Record<string, any[]>;
  CreatedSalesReturnEntries?: any[];
  SalesReturnLinesMap?: Record<string, any[]>;
  VendorMaster: any[];
  ItemGroupMaster: any[];
  ItemMaster: any[];
  WarehouseMaster: any[];
...
function SalesReturn() {
  const API_URL_SAVE = "SalesReturn/0/token";
  const API_URL_EDIT = API_WEB_URLS.MASTER + "/0/token/SalesReturnH/Id";
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        ...
        const API_URL_PE_LIST = API_WEB_URLS.MASTER + "/0/token/SalesReturnData/Id/0";
        ...
        setState((prev) => ({
          ...
          CreatedSalesReturnEntries: peDataArray,
  ...
  const fetchPODataAndPopulateGrid = async (poId: string) => {
    ...
    const lines = prevState.SalesInvoiceLinesMap?.[poId] || [];
    if (lines.length > 0) {
      ...
      const mappedRows: GridRow[] = lines.map((l: any) => ({
        ...
        AvailableQty: 0,
        F_SalesInvoiceH: poId,
        F_SalesInvoiceL: l.SalesInvoiceLId || l.Id || 0,
      }));
      setGridRows(mappedRows);
      
      let newVendorMasterId = prevState.formData.F_VendorMaster;
      const poHeader = prevState.CreatedSalesInvoices?.find((p: any) => String(p.Id) === String(poId));
      if (poHeader && poHeader.F_LedgerMaster) {
         newVendorMasterId = String(poHeader.F_LedgerMaster);
      }
      setState((prev) => ({
        ...prev,
        formData: { ...prev.formData, F_VendorMaster: newVendorMasterId, F_SalesInvoiceH: poId }
      }));
    } else {
  ...
  const fetchPEDataAndPopulateGrid = async (peId: string) => {
    if (!peId) return;
    const prevState = state;
    const pe = prevState.CreatedSalesReturnEntries?.find((p: any) => String(p.Id) === String(peId));
    if (!pe) return;

    let lines: any[] = [];
    try {
      if (pe.SalesReturnLDetails) {
        const parsed = typeof pe.SalesReturnLDetails === "string" ? JSON.parse(pe.SalesReturnLDetails) : pe.SalesReturnLDetails;
        lines = Array.isArray(parsed) ? parsed : [];
      }
    } catch (e) {
      console.error("Error parsing SalesReturnLDetails", e);
    }
    ...
    if (lines.length > 0) {
      ...
      alert("No lines found for the selected Sales Return.");
    }
  };

  const loadSalesReturnRecord = async (id: number) => {
    try {
      ...
      console.error("Error loading Sales Return record:", error);
    }
  };
  ...
  const handleDeletePE = () => {
    if (!state.formData.F_SalesReturnH) return;
    if (window.confirm("Are you sure you want to delete this Sales Return?")) {
      const DELETE_API_URL = `${API_WEB_URLS.MASTER}/0/token/SalesReturnH`;
      Fn_DeleteData(dispatch, () => {}, Number(state.formData.F_SalesReturnH), DELETE_API_URL)
        .then(() => {
  ...
  const handleDownloadPdf = async () => {
    ...
    const htmlString = generateInvoiceHTML("SALES RETURN", state, gridRows, [], taxOverrides);
    ...
    const opt = {
      margin:       5,
      filename:     `SalesReturn_${safeInvoiceNo}.pdf`,
  ...
  const handleSharePDFClick = async () => {
    ...
    try {
      ...
      await navigator.share({
        title: 'Sales Return',
        text: 'Please find attached the Sales Return',
  ...
  const handlePDFExport = async () => {
    ...
    try {
      ...
      const filename = `SalesReturn_${safeInvoiceNo}.pdf`;
  ...
  const SalesReturnCompactStyles = `
    @media (max-width: 991.98px) {
      .sales-return-page .container-fluid { padding: 0.4rem !important; }
      .sales-return-page .card-body { padding: 0.4rem !important; }
      .sales-return-page .card-footer { padding: 0.35rem 0.4rem !important; }
      .sales-return-page .form-label { font-size: 0.75rem; margin-bottom: 0.2rem; }
      .sales-return-page .form-control { font-size: 0.8rem; height: 26px; padding: 0.2rem 0.35rem; }
      .sales-return-page .btn { font-size: 0.8rem; padding: 0.22rem 0.4rem; }
    }
    @media (max-width: 767.98px) {
      .sales-return-page .container-fluid { padding: 0.25rem !important; }
      .sales-return-page .card-body { padding: 0.3rem !important; }
      .sales-return-page .card-footer { padding: 0.25rem 0.3rem !important; }
      .sales-return-page .form-label { font-size: 0.7rem; margin-bottom: 0.15rem; }
      .sales-return-page .form-control { font-size: 0.75rem; height: 24px; padding: 0.15rem 0.28rem; }
      .sales-return-page .btn { font-size: 0.75rem; padding: 0.18rem 0.35rem; }
    }
    .sales-return-print-layout { display: none; }
    @media print {
      @page { margin: 0; }
      body { margin: 0.2cm; line-height: 1.1; }
      body * { visibility: hidden; }
      .sales-return-print-layout, .sales-return-print-layout * { visibility: visible; }
      ...
      .sales-return-print-layout .print-header { text-align: center; margin-bottom: 5px; border-bottom: 1px solid #000; padding-bottom: 2px; }
      .sales-return-print-layout .firm-name { font-size: 18px; font-weight: bold; text-transform: uppercase; margin-bottom: 0; }
      .sales-return-print-layout .print-title { font-size: 14px; font-weight: bold; margin-top: 0; }
      .sales-return-print-layout .print-details { margin-bottom: 5px; }
      .sales-return-print-layout .detail-row { display: flex; justify-content: space-between; margin-bottom: 1px; font-size: 11px; }
      .sales-return-print-layout table { width: 100%; border-collapse: collapse; margin-top: 2px; }
      .sales-return-print-layout th, .sales-return-print-layout td { border: 1px solid #000; padding: 2px 4px; text-align: left; font-size: 11px; }
      .sales-return-print-layout th { background: #eee !important; -webkit-print-color-adjust: exact; }
      .sales-return-print-layout .text-right { text-align: right; }
      .sales-return-print-layout .total-row { font-weight: bold; }
    }
  `;

  return (
    <div className="page-body sales-return-page" style={{ maxWidth: "100%", overflowX: "hidden" }}>
      <style>{SalesReturnCompactStyles}</style>
      <Breadcrumbs mainTitle="Sales Return" parent="Inventory" />
      <Container fluid className="px-2 px-sm-3">
        <Row>
          <Col xs="12">
            <Card>
              <CardHeaderCommon title={`${state.isEditMode ? "Edit" : "Add"} Sales Return`} tagClass="card-title mb-0" />
              <CardBody className="p-2 p-sm-3">
                <Row className="g-2 g-sm-3">
                  <Col md="2">
                    <label className="form-label">Created Sales Return</label>
                    <select className="form-control" value={state.formData.F_SalesReturnH || ""} onChange={(e) => { 
                      const val = e.target.value;
                      handleFormFieldChange("F_SalesReturnH", val); 
                      if (!val) window.location.reload();
                      else fetchPEDataAndPopulateGrid(val); 
                    }}>
                      <option value="">Select PE</option>
                      {state.CreatedSalesReturnEntries?.map((pe: any) => (
