...
function PurchaseReturn() {
    ...
    const fetchMasterData = async () => {
      try {
        ...
        if (recordId) {
          ...
          try {
            const API_ENTRY_NO = API_WEB_URLS.MASTER + "/0/token/GetVoucherNoByVoucherTypeId/Id/13";
  ...
  const fetchPEDataAndPopulateGrid = async (peId: string) => {
    ...
    setState((prev) => ({
      ...
      formData: {
        ...prev.formData,
        PONo: pe.EntryNo || String(pe.EntryNoAuto || ""),
    ...
    if (lines.length > 0) {
      const mappedRows: GridRow[] = lines.map((l: any) => {
        ...
        return {
          ...
          AvailableQty: 0,
          F_PurchaseEntryH: l.F_PurchaseEntryH || 0,
          F_PurchaseEntryL: l.F_PurchaseEntryL || 0,
        };
      });
      setGridRows(mappedRows);
    } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const jsonDataArray = validGridRows.map((row) => {
        ...
        return {
          F_PurchaseEntryL: Number(row.F_PurchaseEntryL) || 0,
      ...
      headerFormData.append("F_PurchaseEntryH", state.formData.F_PurchaseEntryH || "0");
  ...
  const handleDownloadPdf = async () => {
    ...
    const htmlString = generateInvoiceHTML("PURCHASE RETURN", state, gridRows, [], taxOverrides);
    ...
    const opt = {
      margin:       5,
      filename:     `PurchaseReturn_${safeInvoiceNo}.pdf`,
  ...
  const handleSharePDFClick = async () => {
    ...
    try {
      ...
      await navigator.share({
        ...
        text: 'Please find attached the Purchase Return',
  ...
  const handlePDFExport = async () => {
    ...
    try {
      ...
      const filename = `PurchaseReturn_${safeInvoiceNo}.pdf`;
                  ...
                  <Col md="2">
                    ...
                    <select className="form-control" value={state.formData.F_PurchaseReturnH || ""} onChange={(e) => { 
                      ...
                      {state.CreatedPurchaseReturnEntries?.map((pe: any) => (
                        <option key={pe.Id} value={pe.Id}>{pe.EntryNo || pe.EntryNoAuto || pe.Id}</option>
                      ))}
                    </select>
                  </Col>
      ...
      <div className="purchase-return-print-layout">
        <div style={{ border: "1px solid #000", minHeight: "1000px", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ textAlign: "center", borderBottom: "1px solid #000", padding: "10px" }}>
            ...
            <h4 style={{ margin: "5px 0 0 0", fontSize: "16px", textDecoration: "underline" }}>PURCHASE RETURN</h4>
          </div>
